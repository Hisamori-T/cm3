...
export default function QCDSPage() {
  ...
  const [header, setHeader] = useState<Omit<QCDSInput, "direct_works" | "expense_items">>({
    ...
    special_insurance_equipment_rate: 0.000110,
    special_insurance_demolition_rate: 0.019053,
  ...
  const applyQcdsData = useCallback((data: QCDSResponse) => {
    ...
    setHeader({
      ...
      special_insurance_equipment_rate: data.special_insurance_equipment_rate ?? 0.000110,
      special_insurance_demolition_rate: data.special_insurance_demolition_rate ?? 0.019053,
          ...
          <div style={{ padding: "12px 16px" }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "8px 24px" }}>
              {([
                ...
                { label: "特殊保険料率（設備生産物）", key: "special_insurance_equipment_rate", isPct: true },
                { label: "特殊保険料率（解体工事賠責）", key: "special_insurance_demolition_rate", isPct: true },
