...
def export_qcds_excel(project: Any, qcds: Any) -> bytes:
    ...
    for i, w in enumerate(direct_works, 1):
        ...
        _cat_raw = str(getattr(w, "category", "") or "").replace("QCDSCategory.", "")
        _cat_ja  = {"subcontract": "外注", "material": "資材", "other": "その他"}.get(_cat_raw, _cat_raw)
        ws.cell(row, 4, _cat_ja).border = _thin()
